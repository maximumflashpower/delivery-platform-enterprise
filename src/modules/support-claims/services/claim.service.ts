...
export class ClaimService {
  ...
  async findAll(): Promise<Claim[]> {
    return this.claimRepo.find({ where: { deletedAt: IsNull() }, relations: { tickets: true, statusLogs: true } as any, order: { createdAt: 'DESC' } });
  }

  async findById(id: string): Promise<Claim> {
    const c = await this.claimRepo.findOne({ where: { id, deletedAt: IsNull() }, relations: { tickets: true, statusLogs: true } as any });
    if (!c) throw new NotFoundException(`Claim with ID ${id} not found`);
    return c;
  }

  async findByUserId(userId: string): Promise<Claim[]> {
    return this.claimRepo.find({ where: { userId, deletedAt: IsNull() }, relations: { tickets: true } as any, order: { createdAt: 'DESC' } });
  }
